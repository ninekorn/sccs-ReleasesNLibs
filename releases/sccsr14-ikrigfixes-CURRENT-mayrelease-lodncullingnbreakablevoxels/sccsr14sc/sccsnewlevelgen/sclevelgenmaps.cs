...
namespace sccs
{
    public class sclevelgenmaps
    {
        ...
        public int[] buildchunkmaps(Vector3 currentPosition, out int[] mapper, float planeSize_, LevelGenerator4 levelgen_, Vector3 _chunkPos, int tinyChunkWidth_, int tinyChunkHeight_, int tinyChunkDepth_, int typeofterraintiles, sclevelgenclassPrim componentparent_, int levelofdetail_) //, out int vertexNum, out int indicesNum
                ...
                for (int x = 0; x < tinyChunkWidth; x ++)
                {
                    for (int y = 0; y < tinyChunkHeight; y ++)
                    {
                        for (int z = 0; z < tinyChunkDepth; z ++)
                        {
                            ...
                            noiseXZ *= OriginalSimplexNoise.SeamlessNoise((((x * planeSize) + fakepos.X + seed) / _detailScale) * _heightScale, (((z * planeSize) + fakepos.Z + seed) / _detailScale) * _heightScale, 15, 15, 0);
