    ...
    public class sccstriglevelchunkmaps
    {
        ...
        //public void startBuildingArray(Vector4 currentPosition, out sclevelgenchunk.DVertex[] vertexArray, out int[] triangleArray, out int[] mapper, int padding0_, int padding1_, int padding2_, int numberOfObjectInWidth_, int numberOfObjectInHeight_, int numberOfObjectInDepth_, int numberOfInstancesPerObjectInWidth_, int numberOfInstancesPerObjectInHeight_, int numberOfInstancesPerObjectInDepth_, int tinyChunkWidth_, int tinyChunkHeight_, int tinyChunkDepth_, float planeSize_, sclevelgenchunkPrim componentparentprim_, sclevelgenchunk componentparentinstance_, sclevelgenchunk_instances componentparentthischunk_, int fullface_, int voxeltype_)
        {
            ...
            */
                ...
                for (int x = 0; x < tinyChunkWidth; x++)
                {
                    for (int y = 0; y < tinyChunkHeight; y++)
                    {
                        for (int z = 0; z < tinyChunkDepth; z++)
                        {
                            ...
                            noiseXZ *= OriginalSimplexNoise.SeamlessNoise((((x * planeSize) + currentPosition.X + seed) / _detailScalewall) * _heightScalewall, (((z * planeSize) + currentPosition.Z + seed) / _detailScalewall) * _heightScalewall, someperlinoffset, someperlinoffset, 0);
            ...
            {
                Vector3 fakepos = currentPosition;
                fakepos.Y = 0;
                ...
                for (int x = 0; x < tinyChunkWidth; x++)
                {
                    for (int y = 0; y < tinyChunkHeight; y++)
                    {
                        for (int z = 0; z < tinyChunkDepth; z++)
                        {
                            ...
                            /*float noiseXZ = 10;
                            //noiseXZ *= OriginalSimplexNoise.Noise((((x * planeSize) + currentPosition.X + seed) / _detailScale) * _heightScale, (((z * planeSize) + currentPosition.Z + seed) / _detailScale) * _heightScale);
                            noiseXZ *= OriginalSimplexNoise.SeamlessNoise((((x * planeSize) + currentPosition.X + seed) / _detailScale) * _heightScale, (((z * planeSize) + currentPosition.Z + seed) / _detailScale) * _heightScale, someperlinoffset, someperlinoffset, 0);
                ...
                for (int x = 0; x < tinyChunkWidth; x++)
                {
                    for (int y = 0; y < tinyChunkHeight; y++)
                    {
                        for (int z = 0; z < tinyChunkDepth; z++)
                        {
                            ...
                            float noiseXZ = 10;
                            //noiseXZ *= OriginalSimplexNoise.Noise((((x * planeSize) + currentPosition.X + seed) / _detailScalewall) * _heightScale, (((z * planeSize) + currentPosition.Z + seed) / _detailScalewall) * _heightScale);
                            noiseXZ *= OriginalSimplexNoise.SeamlessNoise((((x * planeSize) + currentPosition.X + seed) / _detailScalefloor) * _heightScalefloor, (((z * planeSize) + currentPosition.Z + seed) / _detailScalefloor) * _heightScalefloor, someperlinoffset, someperlinoffset, 0);
                        ...
                        for (int x = 0; x < width; x++)
                        {
                            ...
                            for (int y = 0; y < height; y++)
                            {
                                ...
                                for (int z = 0; z < width; z++)
                                {
                                    ...
                                    noiseValue *= fastNoise.GetNoise((((x * staticPlaneSize) + (currentPosition.X * alternateStaticPlaneSize) + seed) / _detailScalewall) * _heightScalewall, (((y * staticPlaneSize) + (currentPosition.Y * alternateStaticPlaneSize) + seed) / _detailScalewall) * _heightScalewall, (((z * staticPlaneSize) + (currentPosition.Z * alternateStaticPlaneSize) + seed) / _detailScalewall) * _heightScalewall);
                ...
                //for (int j = 0; j < levelgen.frontWall.Count; j++)
                {
                    //if (new Vector3(xChunkPos, yChunkPos, zChunkPos) == levelgen.frontWall[j])
                    {
                        for (int x = 0; x < width; x++)
                        {
                            ...
                            for (int y = 0; y < height; y++)
                            {
                                //float noiseY = Math.Abs((float)(y * planeSize + currentPosition.Y + seed0) /somenoiseval0);6
