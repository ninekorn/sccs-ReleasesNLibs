    ...
    public class sclevelgenchunkshader // : System.Windows.Forms.Application
    {
        ...
        public sccs.sclevelgenchunk.chunkData Renderer(sccs.sclevelgenchunk.chunkData chunkdat) //async
        {
            try
            {
                ...
                Vector3 chunkpos = chunkdat.arrayOfInstancePos[0].position;
                ...
                if (somedraw)
                {
                    if (chunkdat.dVertexData.Length > 0) // chunkdat.arrayOfInstanceVertex[t].Length > 0)
                    {


                        /*if (chunkdat.someswitches[t] == 2)
                        {
                            Program.MessageBox((IntPtr)0, "test2", "scmsg", 0);
                        }*/


                        if (chunkdat.someswitches == 0)
                        {
                            ...
                            VertexBuffer = SharpDX.Direct3D11.Buffer.Create(device, BindFlags.VertexBuffer, chunkdat.dVertexData);
                            ...
                            IndexBuffer = SharpDX.Direct3D11.Buffer.Create(device, BindFlags.IndexBuffer, chunkdat.arrayOfInstanceIndices);
                            /*
                            ...
                            chunkdat.dVertBuffers = VertexBuffer;

                            chunkdat.indexBuffers = IndexBuffer;
                        ...
                        device.ImmediateContext.InputAssembler.SetIndexBuffer(chunkdat.indexBuffers, SharpDX.DXGI.Format.R32_UInt, 0);
                        ...
                        {
                                new VertexBufferBinding(chunkdat.dVertBuffers,Marshal.SizeOf(typeof(sclevelgenchunk.DVertex)), 0),
                            });
                    ...
                    new VertexBufferBinding(instanceBuffer, Utilities.SizeOf<sclevelgenchunk.DInstanceType>(),0), //chunkdat.instanceBuffers[xx+mapWidth*(yy+mapHeight*zz)]
                        ...
                        device.ImmediateContext.DrawIndexedInstanced(chunkdat.arrayOfInstanceIndices.Length, 1, 0, 0, 0);
                        //device.ImmediateContext.DrawIndexed(chunkdat.arrayOfInstanceIndices.Length, 0, 0);
                    }
                }
                chunkdat.someswitches = 1;
