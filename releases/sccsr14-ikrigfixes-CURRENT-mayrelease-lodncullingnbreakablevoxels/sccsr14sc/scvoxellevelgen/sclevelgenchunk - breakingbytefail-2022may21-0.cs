...
namespace sccs
{
    public class sclevelgenchunk
    {
        ...
        public sclevelgenchunk(SharpDX.Direct3D11.Device device, float xi, float yi, float zi, Vector4 color, int width, int height, int depth, Vector3 pos, int mapWidth_, int mapHeight_, int mapDepth_, int tinyChunkWidth_, int tinyChunkHeight_, int tinyChunkDepth_, int mapObjectInstanceWidth_, int mapObjectInstanceHeight_, int mapObjectInstanceDepth_, float planesize_, LevelGenerator4 levelgen, int typeofterraintiles, int somearraycount) //,DInstanceType[] _instances
        {
            ...
            chunkdatlevel = new chunkData[somearraycount]; ;
            ...
            while (enumerator1.MoveNext())
            {
                ...
                chunkdatlevel[ichunkleveltile].arrayOfInstanceIndices = indicesArray0;
                chunkdatlevel[ichunkleveltile].dVertexData = arrayOfD;
                ...
                //Console.WriteLine(chunkPos);
                chunkdatlevel[ichunkleveltile].arrayOfInstancePos = new DInstanceType[1];

                chunkdatlevel[ichunkleveltile].arrayOfInstancePos[0] = new DInstanceType()
                {
                ...
                //chunkdatlevel[ichunkleveltile].indexBuffers = new SharpDX.Direct3D11.Buffer[somearraycount];
                //chunkdatlevel[ichunkleveltile].instanceBuffers = new SharpDX.Direct3D11.Buffer[somearraycount];
                //normalBuffers = new SharpDX.Direct3D11.Buffer[somearraycount];
                //texBuffers = new SharpDX.Direct3D11.Buffer[somearraycount];
                //chunkdatlevel[ichunkleveltile].dVertBuffers = new SharpDX.Direct3D11.Buffer[somearraycount];
                ...
                {
                    Usage = ResourceUsage.Dynamic,
                    SizeInBytes = Marshal.SizeOf(typeof(sclevelgenchunk.DVertex)) * arrayOfD.Length,
                ...
                //arrayoflevelchunk[0].chunkdatlevel[somearrayindex].dVertBuffers.Dispose();
                //arrayoflevelchunk[0].chunkdatlevel[somearrayindex].dVertBuffers = VertexBuffer;
                ...
                var indexbuff = new SharpDX.Direct3D11.Buffer(scgraphics.scdirectx.D3D.device, matrixBufferDescriptionVertex);


                chunkdatlevel[ichunkleveltile].indexBuffers = indexbuff;
                chunkdatlevel[ichunkleveltile].dVertBuffers = VertexBuffer;


                chunkdatlevel[ichunkleveltile].shaderOfChunk = new sclevelgenchunkshader(device, contantBuffer, Layout, VertexShader, PixelShader, GeometryShader, InstanceBuffer, ConstantLightBuffer, somearraycount); //InstanceBuffer

                chunkdatlevel[ichunkleveltile].someswitches = 0;// new int[chunkdatlevel[ichunkleveltile].dVertexData.Length];
            ...
            /*
            for (int i = 0; i < chunkdatlevel.Length; i++)
            {
                for (int v = 0; v < chunkdatlevel[i].dVertexData.Length; v++)
                {

                    chunkdatlevel[i].dVertexData[v].position.X += (arrayofchunks[i].chunkPos.X * (tinyChunkWidth * planesize_));
                    chunkdatlevel[i].dVertexData[v].position.Y += (arrayofchunks[i].chunkPos.Y * (tinyChunkWidth * planesize_));
                    chunkdatlevel[i].dVertexData[v].position.Z += (arrayofchunks[i].chunkPos.Z * (tinyChunkWidth * planesize_));
                }
            }*/
