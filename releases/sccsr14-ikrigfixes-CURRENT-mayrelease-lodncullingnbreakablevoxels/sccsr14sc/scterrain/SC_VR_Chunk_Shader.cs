                ...
                */
                ...
                for (int t = 0; t < total; t++)
                {
                    if (chunkdat.arrayOfInstanceVertex[xx + mapWidth * (yy + mapHeight * zz)].Length > 0)
                    {
                        ...
                        /*
                        device.ImmediateContext.MapSubresource(dVertBuffers[xx + mapWidth * (yy + mapHeight * zz)], MapMode.WriteDiscard, SharpDX.Direct3D11.MapFlags.None, out streamerTWO);
                        streamerTWO.WriteRange(chunkdat.arrayOfInstanceVertex[xx + mapWidth * (yy + mapHeight * zz)], 0, chunkdat.arrayOfInstanceVertex[xx + mapWidth * (yy + mapHeight * zz)].Length);
                        device.ImmediateContext.UnmapSubresource(dVertBuffers[xx + mapWidth * (yy + mapHeight * zz)], 0);
                        streamerTWO.Dispose();*/
