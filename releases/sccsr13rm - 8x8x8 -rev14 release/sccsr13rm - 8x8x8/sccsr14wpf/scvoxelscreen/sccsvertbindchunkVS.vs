...
{ 
	...
	int heightmapmul = currentIndex; // 4
	...
	*/
	...
	someheightmapvalue = someheightmapvalue * 0.0005f;

	float3 somecamerapos = float3(0,0,0);
	float3 otherdir = float3(0,0,-1);//somecamerapos.xyz - (MOVINGPOINT.xyz *-1);  //ovrdirf.xyz * -1;//somecamerapos.xyz - MOVINGPOINT.xyz; 
	MOVINGPOINT.x = MOVINGPOINT.x + (otherdir.x * (-someheightmapvalue) * 0.25f);
	MOVINGPOINT.y = MOVINGPOINT.y + (otherdir.y * (-someheightmapvalue) * 0.25f);
	MOVINGPOINT.z = MOVINGPOINT.z + (otherdir.z * (-someheightmapvalue) * 0.25f);



	/*
	float somemul = someheightmapvalue;
	
	float3 somecamerapos = float3(0,0,1);// float4(0,0,0.5f,1f);
	//somecamerapos  -= rightDir * (128*8*0.0005f * 0.05f); //1.0f or 0.5f
	
	float multowardsrift = somemul * -1.0f;
