...
namespace sccsr15forms
{
    public class sclevelgenclassshader : IDisposable
    {
        ...
        public sclevelgenclassPrim.chunkData Renderer(sclevelgenclassPrim.chunkData chunkdat, int indexBuilt, ShaderResourceView shaderResourceView2D_, int voxeltype_) //async
        {
            ...
            device.ImmediateContext.MapSubresource(chunkdat.dynamicConstantBuffer, MapMode.WriteDiscard, SharpDX.Direct3D11.MapFlags.None, out streamerTWO);
            streamerTWO.WriteRange(chunkdat.matrixBuffer, 0, chunkdat.matrixBuffer.Length);
            device.ImmediateContext.UnmapSubresource(chunkdat.dynamicConstantBuffer, 0);
            streamerTWO.Dispose();

            device.ImmediateContext.MapSubresource(chunkdat.dynamicConstantBuffer, MapMode.WriteDiscard, SharpDX.Direct3D11.MapFlags.None, out mappedResourceLight);
            mappedResourceLight.WriteRange(chunkdat.lightBuffer, 0, chunkdat.lightBuffer.Length);
            device.ImmediateContext.UnmapSubresource(chunkdat.dynamicConstantBuffer, 0);
            mappedResourceLight.Dispose();


            if (chunkdat.switchForRender == 2)
            {
