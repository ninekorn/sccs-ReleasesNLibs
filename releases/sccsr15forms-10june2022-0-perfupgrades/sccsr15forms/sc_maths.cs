...
namespace sccsr15forms
{
    public static class sc_maths
    {
        ...
        static Vector3 dirnodeatob;
        static float somedotf;

        //,float vl, float vr, float vu,float vd, float vb, float vf
        public static float checkdot(Vector3 dirf, Vector3 nodeA, Vector3 nodeB)//.
        {
            dirnodeatob = nodeA - nodeB;
            dirnodeatob.Normalize();

            somedotf = Dot(dirnodeatob.X, dirnodeatob.Z, dirf.X, dirf.Z);
        ...
        static Vector3 pointBoffsettofrontofplayer;
        static float somedotr;
        static float dist;
        static Vector3 pointfront;
        //,float vl, float vr, float vu,float vd, float vb, float vf
        public static float distancebasedondirection(Vector3 dirf, Vector3 dirr, Vector3 nodeA, Vector3 nodeB, float minx, float miny, float minz, float diagminx, float diagminy, float diagminz, float diagmaxx, float diagmaxy, float diagmaxz) // i was thinking about using the index instead and then was like well i need the distance man.
        {
            ...
            somedotf = Dot(dirnodeatob.X, dirnodeatob.Z, dirf.X, dirf.Z);
            somedotr = Dot(dirnodeatob.X, dirnodeatob.Z, dirr.X, dirr.Z);

            dist =  sc_check_distance_node_3d_geometry(nodeA, nodeB, 1,1,1,1,1,1); //Vector3.DistanceSquared(nodeA, nodeB); //Vector3.Distance(nodeA,nodeB);//
            pointBoffsettofrontofplayer = Vector3.Zero;
            ...
            //dotprodlr = somedotr;
            //dotprodfb = somedotf;
            //Vector3.Distance(nodeA, pointBoffsettofrontofplayer);// 
            return sc_check_distance_node_3d_geometry(nodeA, pointBoffsettofrontofplayer, 1, 1, 1, 1, 1, 1);// Vector3.DistanceSquared(nodeA, pointBoffsettofrontofplayer);
            ...
            return Vector3.DistanceSquared(nodeA, pointBoffsettofrontofplayer);*/
            //MY WORKING VERSION 
