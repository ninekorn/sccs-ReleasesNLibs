...
namespace sccs
{
    public class chunkscreen : IDisposable
    {
        ...
        Vector4 chunkinitpos;


        //int maxveclength = 8; // 4 or 8
        int maxveclength = 4; // 4 or 8
       ...
       // FastNoise fastNoise = new FastNoise();
            ...
            out double m21d, out double m22d, out double m23d, out double m24d, out double m31d, out double m32d, out double m33d, out double m34d, out double m41d, out double m42d, out double m43d, out double m44d, int someswtc)// , int somechunkkeyboardpriminstanceindex_, int chunkprimindex_, int chunkinstindex_
        {
            ...
            chunkinitpos = currentPosition;



            _vert_offset_x = 0.0f;// vert_offset_x;
            _vert_offset_y = 0.0f;//vert_offset_y;
            _vert_offset_z = 0.0f;// vert_offset_z;
            ...
            padding0 = numberOfInstancesPerObjectInWidth;
            padding1 = numberOfInstancesPerObjectInHeight;
            padding2 = numberOfInstancesPerObjectInDepth;
            ...
            {
                staticPlaneSize = planeSize;
                alternateStaticPlaneSize = planeSize * 100;
            }
            ...
            //alternateStaticPlaneSize = planeSize;
            //staticPlaneSize = planeSize;
                    ...
                    for (int y = 0; y < height; y++)
                    {
                        for (int z = 0; z < depth; z++)
                        {
                            ...
                            else if (swtcsetneighboors == -1)
                            {
                                ...
                                //noiseXZ *= fastNoise.GetNoise((((x * staticPlaneSize) + (currentPosition.X * alternateStaticPlaneSize)) / _detailScale) * _HeightScale, (((y * staticPlaneSize) + (currentPosition.Y * alternateStaticPlaneSize)) / _detailScale) * _HeightScale, (((z * staticPlaneSize) + (currentPosition.Z * alternateStaticPlaneSize)) / _detailScale) * _HeightScale);
                                //noiseXZ = fastNoise.GetNoise(((x * staticPlaneSize) / _detailScale) * _HeightScale, ((y * staticPlaneSize) / _detailScale) * _HeightScale, ((z * staticPlaneSize) / _detailScale) * _HeightScale);
                                //noiseXZ *= fastNoise.GetNoise((((currentPosition.X + x) * staticPlaneSize) / _detailScale) * _HeightScale, (((currentPosition.Y + y) * staticPlaneSize) / _detailScale) * _HeightScale, (((currentPosition.Z + z) * staticPlaneSize) / _detailScale) * _HeightScale);
            ...
            if (tBytes < totalBytes)
            {
                ...
                if (indexBytes < totalBytes)
                {
                    ...
                    */
                    ...
                    /*if (IsTransparent(xi, yi - 1, zi))
                    {
                        buildBottomFace(xi, yi, zi, 5.0f);
                    }*/
                    /*if (IsTransparent(xi - 1, yi, zi))
                    {
                        buildTopLeft(xi, yi, zi, 2.0f);
                    }  */            
                    /*if (IsTransparent(xi + 1, yi, zi))
                    {
                        buildTopRight(xi, yi, zi, 3.0f);
                    }               
                    if (IsTransparent(xi, yi, zi + 1))
                    {
                        buildFrontFace(xi, yi, zi, 0.0f);
                    }
                    //BACK FACE GENERATION WITH NEIGHBOOR CHECK. NEIGHBOOR CHECK BYTES NOT WORKING ENTIRELY.
                    if (IsTransparent(xi, yi, zi - 1))
                    {
                        buildBackFace(xi, yi, zi, 4.0f);
                    }*/
        ...
        void buildTopFace(int xi, int yi, int zi, float typeofface) //int _x, int _y, int _z, Vector3 chunkPos
        {
            _maxWdth = width;
            _maxDepth = depth;
            _maxHght = height;
            ...
            if (_block == 1) //|| _block == 2
            {
                //if (IsTransparent(temptopfacexi, temptopfaceyi + 1, temptopfacezi))
                {
                    for (int _xx = 0; _xx < _maxWdth; _xx++)
                    {
                        ...
                        for (int _zz = 0; _zz < _maxDepth; _zz++)
                        {
                            ...
                            if (rowIterateX < width && rowIterateZ < depth)
                            {
                                ...
                                //if (someswtc == 1)
                                {
                                    if (_xx == 0 && _zz == 0)//&& foundVertOne == false && foundVertThree == false
                                    {
                                        ...
                                        //map[rowIterateX + width * (rowIterateY + height * rowIterateZ)] = 2;
                            ...
                            {
                                _tempChunkArray[(rowIterateX) + width * (yi + height * (rowIterateZ))] = 1;
                    ...
                    if (getChunkVertexByte(oneVertIndexX, oneVertIndexY, oneVertIndexZ) == 0)
                    {
                        ...
                        {
                            position = new Vector4((oneVertIndexX + _vert_offset_x) * planeSize, (oneVertIndexY + _vert_offset_y) * planeSize, (oneVertIndexZ + _vert_offset_z) * planeSize, 0),
                            indexPos = new Vector4(xi, yi, zi, typeofface),
                            color = chunkcolor,
                        ...
                        //map[xi + width * (yi + height * zi)] = 2;
                        //mapindex0 = (width-1 -rowIterateX) + width * ((yi) + height * rowIterateZ);
                        mapindex0 = xi + width * (yi + height * zi);
                    ...
                    if (getChunkVertexByte(twoVertIndexX, twoVertIndexY, twoVertIndexZ) == 0)
                    {
                        vertexlist.Add(new SC_instancedChunk.DVertex()
                        {
                            position = new Vector4((twoVertIndexX + _vert_offset_x) * planeSize, (twoVertIndexY + _vert_offset_y) * planeSize, (twoVertIndexZ + _vert_offset_z) * planeSize, 1),
                            indexPos = new Vector4(xi, yi, zi, typeofface),
                            color = chunkcolor,
                            normal = new Vector3(0, 1, 0),
                            padding0 = padding0,
                        ...
                        //mapindex1 = (width - 1 - rowIterateX) + width * ((rowIterateY) + height * (depth-1- rowIterateZ));
                        //mapindex1 = (rowIterateX) + width * ((yi) + height * (rowIterateZ));
                        //map[xi + width * (yi + height * zi)] = 2;
                        mapindex1 = (rowIterateX) + width * ((yi) + height * (rowIterateZ));
                    ...
                    if (getChunkVertexByte(threeVertIndexX, threeVertIndexY, threeVertIndexZ) == 0)
                    {
                        vertexlist.Add(new SC_instancedChunk.DVertex()
                        {
                            position = new Vector4((threeVertIndexX + _vert_offset_x) * planeSize, (threeVertIndexY + _vert_offset_y) * planeSize, (threeVertIndexZ + _vert_offset_z) * planeSize, 2),
                            indexPos = new Vector4(xi, yi, zi, typeofface),
                            color = chunkcolor,
                            normal = new Vector3(0, 1, 0),
                            padding0 = padding0,
                        ...
                        //mapindex2 = (width-1-rowIterateX) + width * ((rowIterateY) + height * (threeVertIndexZ));
                        // mapindex2 = ((width - 1 - rowIterateX)) + width * ((threeVertIndexY) + height * (depth-1-rowIterateZ));
                        //mapindex2 = ((width - 1 - rowIterateX)+xi) + width * ((yi) + height * ((zi)));
                        //mapindex2 = ((threeVertIndexX) + xi) + width * ((yi) + height * ((zi)));
                        //mapindex2 = (rowIterateX) + width * ((yi) + height * (rowIterateZ));
                        mapindex2 = (width - 1 - rowIterateX) + width * ((yi) + height * (depth - 1 - rowIterateZ));
                    ...
                    if (getChunkVertexByte(fourVertIndexX, fourVertIndexY, fourVertIndexZ) == 0)
                    {
                        vertexlist.Add(new SC_instancedChunk.DVertex()
                        {
                            position = new Vector4((fourVertIndexX + _vert_offset_x) * planeSize, (fourVertIndexY + _vert_offset_y) * planeSize, (fourVertIndexZ + _vert_offset_z) * planeSize, 3),
                            indexPos = new Vector4(xi, yi, zi, typeofface),
                            color = chunkcolor,
                            normal = new Vector3(0, 1, 0),
                            padding0 = padding0,
                        ...
                        mapindex3 = (rowIterateX) + width * ((yi) + height * (depth-1- rowIterateZ));
                        //map[xi + width * (yi + height * zi)] = 2;
                    ...
                    if (getChunkVertexByte(oneVertIndexX, oneVertIndexY, oneVertIndexZ) == 1 && getChunkVertexByte(twoVertIndexX, twoVertIndexY, twoVertIndexZ) == 1 && getChunkVertexByte(threeVertIndexX, threeVertIndexY, threeVertIndexZ) == 1 && getChunkVertexByte(fourVertIndexX, fourVertIndexY, fourVertIndexZ) == 1)
                    {
                        ...
                        /*
                        _chunkVertexArray[oneVertIndexX + vertexlistWidth * ((oneVertIndexY) + vertexlistHeight * oneVertIndexZ)] = 2;
                        _chunkVertexArray[twoVertIndexX + vertexlistWidth * ((twoVertIndexY) + vertexlistHeight * twoVertIndexZ)] = 2;
                        _chunkVertexArray[threeVertIndexX + vertexlistWidth * ((threeVertIndexY) + vertexlistHeight * threeVertIndexZ)] = 2;
                        _chunkVertexArray[fourVertIndexX + vertexlistWidth * ((fourVertIndexY) + vertexlistHeight * fourVertIndexZ)] = 2;
                        */
        ...
        void buildTopLeft(int xi, int yi, int zi, float typeofface) //int _x, int _y, int _z, Vector3 chunkPos
        {
            _maxWdth = width;
            _maxDepth = depth;
            _maxHght = height;
                ...
                if (IsTransparent(xi - 1, yi, zi))
                {
                    for (int _yy = 0; _yy < _maxHght; _yy++)
                    {
                        ...
                        for (int _zz = 0; _zz < _maxDepth; _zz++)
                        {
                            ...
                            {
                                _tempChunkArrayLeftFace[(xi) + width * (rowIterateY + height * (rowIterateZ))] = 1;
                                ////////Instantiate(_blockZero, new Vector3(rowIterateX + 0.5f, y, rowIterateZ + 0.5f) * planeSize + chunkPos, Quaternion.identity);
                            }
                        }
                    }
        ...
        void buildTopRight(int xi, int yi, int zi, float typeofface) //int xi, int _y, int _z, Vector3 chunkPos
        {
            _maxWdth = width;
            _maxDepth = depth;
            _maxHght = height;
                ...
                if (IsTransparent(xi + 1, yi, zi))
                {
                    for (int _yy = 0; _yy < _maxHght; _yy++)
                    {
                        ...
                        for (int _zz = 0; _zz < _maxDepth; _zz++)
                        {
                            ...
                            if (blockExistsInArray(xi, rowIterateY, rowIterateZ))
                            {
                                _tempChunkArrayRightFace[(xi) + width * (rowIterateY + height * (rowIterateZ))] = 1;
                                ////////Instantiate(_blockZero, new Vector3(rowIterateX + 0.5f, y, rowIterateZ + 0.5f) * planeSize + chunkPos, Quaternion.identity);
                            }
                        }
                    }
                    ...
                    if (getChunkVertexByte(oneVertIndexX, oneVertIndexY, oneVertIndexZ) == 1 && getChunkVertexByte(twoVertIndexX, twoVertIndexY, twoVertIndexZ) == 1 && getChunkVertexByte(threeVertIndexX, threeVertIndexY, threeVertIndexZ) == 1 && getChunkVertexByte(fourVertIndexX, fourVertIndexY, fourVertIndexZ) == 1)
                    {
                        ...
                        */
                        ...
                        /*
                        Console.WriteLine("i0:" + _index0 + "/i1:" + _index1 + "/i2:" + _index2 + "/i3:" + _index3);
        ...
        void buildFrontFace(int xi, int yi, int zi, float typeofface) // int _x, int _y, int _z, Vector3 chunkPos
        {
                ...
                if (IsTransparent(xi, yi, zi + 1))
                {
                    for (int _yy = 0; _yy < _maxHght; _yy++)
                    {
                        ...
                        for (int _xx = 0; _xx < _maxWdth; _xx++)
                        {
                            ...
                            {
                                _tempChunkArrayFrontFace[(rowIterateX) + width * (rowIterateY + height * (zi))] = 1;
                    ...
                    if (getChunkVertexByte(fourVertIndexX, fourVertIndexY, fourVertIndexZ) == 0)
                    {
                        vertexlist.Add(new SC_instancedChunk.DVertex()
                        {
                            position = new Vector4((fourVertIndexX + _vert_offset_x) * planeSize, (fourVertIndexY + _vert_offset_y) * planeSize, (fourVertIndexZ + _vert_offset_z) * planeSize, 2),
                            indexPos = new Vector4(xi, yi, zi, typeofface),
                            color = chunkcolor,
                            normal = new Vector3(0, 0, 1),
                            padding0 = padding0,
                    ...
                    if (getChunkVertexByte(threeVertIndexX, threeVertIndexY, threeVertIndexZ) == 0)
                    {
                        vertexlist.Add(new SC_instancedChunk.DVertex()
                        {
                            position = new Vector4((threeVertIndexX + _vert_offset_x) * planeSize, (threeVertIndexY + _vert_offset_y) * planeSize, (threeVertIndexZ + _vert_offset_z) * planeSize, 3),
                            indexPos = new Vector4(xi, yi, zi, typeofface),
                            color = chunkcolor,
                            normal = new Vector3(0, 0, 1),
                            padding0 = padding0,
        ...
        void buildBackFace(int xi, int yi, int zi, float typeofface) //int _x, int _y, int zi, Vector3 chunkPos
        {
            _maxWdth = width;
            _maxDepth = depth;
            _maxHght = height;
                ...
                if (IsTransparent(xi, yi, zi - 1))
                {
                    for (int _yy = 0; _yy < _maxHght; _yy++)
                    {
                        ...
                        for (int _xx = 0; _xx < _maxWdth; _xx++)
                        {
                            ...
                            if (blockExistsInArray(rowIterateX, rowIterateY, zi))
                            {
                                _tempChunkArrayBackFace[(rowIterateX) + width * (rowIterateY + height * (zi))] = 1;
                                //////Instantiate(_blockZero, new Vector3(rowIterateX + 0.5f, y, rowIterateZ + 0.5f) * planeSize + _chunkPos, Quaternion.identity);
                            }
                        }
                    }
        ...
        void buildBottomFace(int xi, int yi, int zi, float typeofface) //int _x, int _y, int _z, Vector3 chunkPos
        {
            _maxWdth = width;
            _maxDepth = depth;
            _maxHght = height;
                ...
                if (IsTransparent(xi, yi - 1, zi))
                {
                    for (int _xx = 0; _xx < _maxWdth; _xx++)
                    {
                        ...
                        for (int _zz = 0; _zz < _maxDepth; _zz++)
                        {
                            ...
                            if (blockExistsInArray(rowIterateX, yi, rowIterateZ))
                            {
                                _tempChunkArrayBottomFace[(rowIterateX) + width * (yi + height * (rowIterateZ))] = 1;
                                //////Instantiate(_blockZero, new Vector3(rowIterateX + 0.5f, y, rowIterateZ + 0.5f) * planeSize + chunkPos, Quaternion.identity);
                            }
                        }
                    }
