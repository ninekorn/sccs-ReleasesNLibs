...
namespace sccs
{
    public class sclevelgenchunk
    {
        ...
        public static sccstriglevelchunk[] arrayofchunks;
        public static sccstriglevelchunkmaps[] arrayofchunksmaps;
        ...
        public sclevelgenchunk(SharpDX.Direct3D11.Device device, float xi, float yi, float zi, Vector4 color, int width, int height, int depth, Vector3 pos, int mapWidth_, int mapHeight_, int mapDepth_, int tinyChunkWidth_, int tinyChunkHeight_, int tinyChunkDepth_, int mapObjectInstanceWidth_, int mapObjectInstanceHeight_, int mapObjectInstanceDepth_, float planesize_, LevelGenerator4 levelgen, int typeofterraintiles, int somearraycount) //,DInstanceType[] _instances
        {
            ...
            while (enumerator1.MoveNext())
            {
                ...
                int[] tester = sometrigchunkmap.buildchunkmaps(newchunkpos, out somemap, planeSize, levelgen, chunkPos, tinyChunkWidth, tinyChunkHeight, tinyChunkDepth, typeofterraintile, this);      
            ...
            while (enumerator1.MoveNext())
            {
                ...
                int[] tester = arrayofchunks[ichunkleveltile].startBuildingArray(newchunkpos, out vertexArray0, out indicesArray0, out somemap, out arrayOfD, out normals, out texturesCoordinates, planeSize, levelgen, chunkPos, tinyChunkWidth, tinyChunkHeight, tinyChunkDepth, typeofterraintile, this, arrayofchunksmaps[ichunkleveltile].map);
            ...
            for (int i = 0; i < somearraycount; i++)
            {
                sometrigchunkmap = new sccstriglevelchunkmaps();
                int[] tester = sometrigchunkmap.buildchunkmaps(pos, out somemap, planeSize, levelgen, _chunkPos, tinyChunkWidth, tinyChunkHeight, tinyChunkDepth, typeofterraintiles, this);
                arrayofchunksmaps[i] = sometrigchunkmap;
            }

            for (int i = 0; i < somearraycount; i++)
            {
                sometrigchunkmap = new sccstriglevelchunkmaps();
                int[] tester = newChunker.startBuildingArray(pos, out vertexArray0, out indicesArray0, out somemap, out arrayOfD, out normals, out texturesCoordinates, planeSize, levelgen, _chunkPos, tinyChunkWidth, tinyChunkHeight, tinyChunkDepth, typeofterraintiles, this, arrayofchunksmaps[someindex].map);


                arrayofchunks[i] = newChunker;


                arrayOfInstanceVertex[i] = vertexArray0; //new Vector4(vertexArray0[v].X, vertexArray0[v].Y, vertexArray0[v].Z, 1);
                arrayOfInstanceIndices[i] = indicesArray0;
                arrayOfDVertex[i] = arrayOfD;
                arrayOfInstanceNormals[i] = normals;
                arrayOfInstanceTexturesCoordinates[i] = texturesCoordinates;

                //instances[i] = new Vector4[1];
                //instances[i][0] = new Vector4(position.X, position.Y, position.Z, 1);

                instances[i] = new DInstanceType()
                {
                    position = pos,
                };
            ...
            for (int x = 0; x < mapWidth; x++)
            {
                for (int y = 0; y < mapHeight; y++)
                {
                    for (int z = 0; z < mapDepth; z++)
                    {
                        ...
                        int someindex = xx + mapWidth * (yy + mapHeight * zz);
                        ...
                        //byte[] tester = newChunker.startBuildingArray(position, out vertexArray0, out indicesArray0, out somemap, out arrayOfD, out normals, out texturesCoordinates);
                        //int[] tester = newChunker.startBuildingArray(position, out vertexArray0, out indicesArray0, out somemap, out arrayOfD, out normals, out texturesCoordinates, planeSize, levelgen, _chunkPos, tinyChunkWidth, tinyChunkHeight, tinyChunkDepth, typeofterraintiles, this);

                        int[] tester = sometrigchunkmap.buildchunkmaps(position, out somemap, planeSize, levelgen, _chunkPos, tinyChunkWidth, tinyChunkHeight, tinyChunkDepth, typeofterraintiles, this);

                        arrayofchunksmaps[someindex] = sometrigchunkmap;
                    }
                }
            }
            ...
            for (int x = 0; x < mapWidth; x++)
            {
                for (int y = 0; y < mapHeight; y++)
                {
                    for (int z = 0; z < mapDepth; z++)
                    {
                        ...
                        //byte[] tester = newChunker.startBuildingArray(position, out vertexArray0, out indicesArray0, out somemap, out arrayOfD, out normals, out texturesCoordinates);
                        int[] tester = newChunker.startBuildingArray(position, out vertexArray0, out indicesArray0, out somemap, out arrayOfD, out normals, out texturesCoordinates, planeSize, levelgen, _chunkPos, tinyChunkWidth, tinyChunkHeight, tinyChunkDepth, typeofterraintiles, this, arrayofchunksmaps[someindex].map);
                        ...
                        arrayofchunks[someindex] = newChunker;
        ...
        public sccstriglevelchunk getChunk(float x, float y, float z) //, Vector3 chunkPos
        {
            ...
            for (int i = 0; i < arrayofchunks.Length; i++)
            {
                ...
                float x1 = (float)(Math.Round(arrayofchunks[i].chunkPos.X * 10) / 10);
                float y1 = (float)(Math.Round(arrayofchunks[i].chunkPos.Y * 10) / 10);
                float z1 = (float)(Math.Round(arrayofchunks[i].chunkPos.Z * 10) / 10);
