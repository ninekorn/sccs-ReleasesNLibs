...
namespace sccsr15forms
{
    public class sclevelgenmaps
    {
        ...
        //public static Dictionary<sclevelgenmaps, Vector3> chunkz = new Dictionary<sclevelgenmaps, Vector3>();

        public int levelofdetail = 1;
        public int[] buildchunkmaps(Vector3 currentPosition, out int[] mapper, float planeSize_, LevelGenerator4 levelgen_, Vector3 _chunkPos, int typeofterraintiles, sclevelgenclassPrim componentparent_, int levelofdetail_) //, out int vertexNum, out int indicesNum
        //public void startBuildingArray(Vector4 currentPosition, out sclevelgenchunk.DVertex[] vertexArray, out int[] triangleArray, out int[] mapper, int padding0_, int padding1_, int padding2_, int numberOfObjectInWidth_, int numberOfObjectInHeight_, int numberOfObjectInDepth_, int numberOfInstancesPerObjectInWidth_, int numberOfInstancesPerObjectInHeight_, int numberOfInstancesPerObjectInDepth_, int tinyChunkWidth_, int tinyChunkHeight_, int tinyChunkDepth_, float planeSize_, sclevelgenchunkPrim componentparentprim_, sclevelgenchunk componentparentinstance_, sclevelgenchunk_instances componentparentthischunk_, int fullface_, int voxeltype_)
        {
            ...
            */
            ...
            /*
            width = tinyChunkWidth_ / levelofdetail_;
            height = tinyChunkHeight_ / levelofdetail_;
            depth = tinyChunkDepth_ / levelofdetail_;*/
            ...
            float currentratio = standardwidth / tinyChunkWidth;

            //Console.WriteLine(currentratio);
            planeSize = 0.1f * currentratio; //0.05f when 20w20h20d
            ...
            */
                ...
                for (int x = 0; x < tinyChunkWidth; x++)
                {
                    for (int y = 0; y < tinyChunkHeight; y ++)
                    {
                        for (int z = 0; z < tinyChunkDepth; z ++)
                        {
                            ...
                            noiseXZ *= OriginalSimplexNoise.SeamlessNoise((((x * planeSize) + chunkPos.X + seed) / _detailScale) * _heightScale, (((z * planeSize) + chunkPos.Z + seed) / _detailScale) * _heightScale, 15, 15, 0);


                            if (chunkPos.Y == LevelGenerator4.wallheightsize)
                            {
                            ...
                            if (y == height-1 && chunkPos.Y == LevelGenerator4.wallheightsize)
                            {
                                map[x + width * (y + height * z)] = 1;
                            }
            ...
            if (typeofterraintiles != 15 && typeofterraintiles != 0 && chunkPos.Y != 0)
            {
                for (int x = 0; x < tinyChunkWidth; x++)
                {
                    for (int y = 0; y < tinyChunkHeight; y++)
                    {
                        for (int z = 0; z < tinyChunkDepth; z++)
                        {
                            ...
                            float noiseXZ = somenoisevalue;
                            //noiseXZ *= OriginalSimplexNoise.Noise((((x * planeSize) + chunkPos.X + seed) / _detailScale) * _heightScale, (((z * planeSize) + chunkPos.Z + seed) / _detailScale) * _heightScale);
                            noiseXZ *= OriginalSimplexNoise.SeamlessNoise((((x * planeSize) + chunkPos.X + seed) / _detailScaleceiling) * _heightScaleceiling, (((z * planeSize) + chunkPos.Z + seed) / _detailScaleceiling) * _heightScaleceiling, 15, 15, 0);
                ...
                for (int x = 0; x < tinyChunkWidth; x ++)
                {
                    for (int y = 0; y < tinyChunkHeight; y ++)
                    {
                        for (int z = 0; z < tinyChunkDepth; z ++)
                        {
                            ...
                            noiseXZ *= OriginalSimplexNoise.SeamlessNoise((((x * planeSize) + fakepos.X + seed) / _detailScaleceiling) * _heightScaleceiling, (((z * planeSize) + fakepos.Z + seed) / _detailScaleceiling) * _heightScaleceiling, 15, 15, 0);
                        ...
                        for (int x = 0; x < width; x ++)
                        {
                            ...
                            for (int y = 0; y < height; y ++)
                            {
                                ...
                                for (int z = 0; z < width; z ++)
                                {
                                    ...
                                    noiseValue *= fastNoise.GetNoise((((x * staticPlaneSize) + (chunkPos.X * alternateStaticPlaneSize) + seed) / _detailScale) * _heightScale, (((y * staticPlaneSize) + (chunkPos.Y * alternateStaticPlaneSize) + seed) / _detailScale) * _heightScale, (((z * staticPlaneSize) + (chunkPos.Z * alternateStaticPlaneSize) + seed) / _detailScale) * _heightScale);
                    ...
                    //if (new Vector3(xChunkPos, yChunkPos, zChunkPos) == levelgen.builtLeftFrontInsideCorner[j])
                    {
                        for (int x = 0; x < width; x++)
                        {
                            ...
                            for (int y = 0; y < height; y++)
                            {
                                ...
                                {
                                    //float noiseZ = Math.Abs((float)(z * planeSize + chunkPos.Z + seed0) /somenoiseval0);
                                    float noiseZ2 = Math.Abs((float)((depth - 1 - z) * planeSize + chunkPos.Z + seed0) / somenoiseval1);
                                    float noiseZ5 = Math.Abs((float)((depth - 1 - z) * planeSize + chunkPos.Z + seed0) / somenoiseval1);
            ...
            if (typeofterraintiles == 6)
            {
                //for (int j = 0; j < levelgen.builtRightFrontInsideCorner.Count; j++)
                {
                    //if (new Vector3(xChunkPos, yChunkPos, zChunkPos) == levelgen.builtRightFrontInsideCorner[j])
                    {
                        ...
                        {
                            //float noiseX = Math.Abs((float)(x * planeSize + chunkPos.X + seed0) /somenoiseval0);
                            float noiseX2 = Math.Abs((float)((width - 1 - x) * planeSize + chunkPos.X + seed0) / somenoiseval1);
                            float noiseX5 = Math.Abs((float)((width - 1 - x) * planeSize + chunkPos.X + seed0) / somenoiseval1);
                            for (int y = 0; y < height; y++)
                            {
                                //float noiseY = Math.Abs((float)(y * planeSize + chunkPos.Y + seed0) /somenoiseval0);
                                float noiseY2 = Math.Abs((float)(y * planeSize + chunkPos.Y + seed0) / somenoiseval1);
                                ...
                                for (int z = 0; z < width; z++)
                                {
                                    ...
                                    float noiseValue = someothernoisevalue;
                                    noiseValue *= fastNoise.GetNoise(((((width - 1 - x) * staticPlaneSize) + (chunkPos.X * alternateStaticPlaneSize) + seed) / _detailScale) * _heightScale, (((y * staticPlaneSize) + (chunkPos.Y * alternateStaticPlaneSize) + seed) / _detailScale) * _heightScale, (((z * staticPlaneSize) + (chunkPos.Z * alternateStaticPlaneSize) + seed) / _detailScale) * _heightScale);
