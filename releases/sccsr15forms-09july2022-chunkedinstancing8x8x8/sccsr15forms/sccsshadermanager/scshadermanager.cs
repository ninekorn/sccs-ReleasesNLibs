...
namespace sccsr15forms
{
    public class SC_ShaderManager                 // 77 lines
    {
        ...
        public int createcubeshaders(Device device, IntPtr windowsHandle, int createikrig, int shadertype)
        {
            ...
            shaderindex++;
            Console.WriteLine(shaderindex);
            return shaderindex;
        }


        //public tutorialfacemesh[] arrayoffacemeshtop;




        public tutorialfacemeshshader tutorialchunkinstmeshshader { get; set; }



        public void createchunkinstmesh(Device device, int facetype)
        {
        ...
        public bool Initialize(Device device, IntPtr windowsHandle, int createikrig) //, float x, float y, float z, Vector4 color,Matrix worldMatrix
        {
            ...
            tutorialchunkinstmeshshader = new tutorialfacemeshshader(device, bytecodevert, bytecodepix);
            ...
            lightBufferDesc = new BufferDescription()
            {
                Usage = ResourceUsage.Dynamic,
                SizeInBytes = Utilities.SizeOf<SC_cube.DLightBuffer>(),
                BindFlags = BindFlags.ConstantBuffer,
            ...
            if (Program.activatenotyetinfinitevoxelterrain == 1)
            {
                ...
                ConstantLightBuffer00 = new SharpDX.Direct3D11.Buffer(device, lightBufferDesc);
                _this_object_texture_shader = new SC_cube_shader_final();
                _this_object_texture_shader.Initialize(device, windowsHandle, ConstantLightBuffer00, _DLightBuffer_cube, 1);
                //////////////////////
                //SC PHYSICS CUBES
                //////////////////////
            ...
            if (createikrig == 1)
            {
                ...
                if (device == null)
                {
                    Program.MessageBox((IntPtr)0, "error", "sccsmsg", 0);
                }
        ...
        public bool Rendertutorialchunkinstmesh(DeviceContext deviceContext, Matrix worldMatrix, Matrix viewMatrix, Matrix projectionMatrix, ShaderResourceView texture, tutorialcubeaschunkinst somecubeaschunkinst, tutorialfacemesh tutorialfacemesh, updatePrim.worldviewprobuffer worldViewProjbuffer)
        {
            tutorialchunkinstmeshshader.Render(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, somecubeaschunkinst, worldViewProjbuffer, tutorialfacemesh);
            return true;
        }
