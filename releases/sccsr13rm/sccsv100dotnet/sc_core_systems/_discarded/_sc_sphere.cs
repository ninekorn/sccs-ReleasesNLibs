...
{
    public class _sc_sphere : ITransform, IComponent
    {
        ...
        public _sc_core_systems.SC_Graphics._sc_sphere.DInstanceData[] instancesDataRIGHT { get; set; }
        public _sc_core_systems.SC_Graphics._sc_sphere.DInstanceData[] instancesDataUP { get; set; }
        ...
        public _sc_sphere_shader_final _this_object_texture_shader { get; set; }

        DLightBuffer[] _DLightBuffer = new DLightBuffer[1];

        _sc_sphere_instances[] _arrayOfInstances;
        ...
        public _sc_sphere_instances _singleObjectOnly;
        ...
        private bool InitializeBuffer(_sc_console_directx D3D, float _sizeX, float _sizeY, float _sizeZ, float tileSize, int instX, int instY, int instZ, IntPtr windowsHandle, Matrix matroxer, int isTerrain, float offsetPosX, float offsetPosY, float offsetPosZ)
        {
            try
            {
                ...
                CreateSphere sphere = new CreateSphere(1, 24, 16, out verts, out trigs, out normals);

                //List<Vector3> verts;// = new List<Vector3>();
                //List<int> trigs;// = new List<int>();
                //List<Vector3> normals;

                //sphere.cre(0.01f, 3, 3, out verts, out trigs, out normals);
                ...
                sphere.OnWizardCreate(0.01f, 3, 3, out verts, out trigs, out normals);
                ...
                for (int i = 0; i < trigs.Count; i++)
                {
                    triangles[i] = trigs[trigs.Count - i - 1]; //trigs[i]; //
                }*/
                ...
                _arrayOfInstances = new _sc_sphere_instances[instX * instY * instZ];
                ...
                for (int x = 0; x < instX; x++)
                {
                    for (int y = 0; y < instY; y++)
                    {
                        for (int z = 0; z < instZ; z++)
                        {
                            ...
                            _sc_sphere_instances _cube = new _sc_sphere_instances();
