...
public class SC_AI //: MonoBehaviour
{
    ...
    void Starter()
    {
        ...
        perc = new Perceptron.Perceptron(3, 0.05f);
    ...
    Vector2 getFormationWaypoint(float offsetX, float offsetY)
    {
        ...
        var pointRightX = coordsPlayer.x + ((rightDirPlayer.x * offsetX));
        var pointRightY = coordsPlayer.y + ((rightDirPlayer.y * offsetY));
    ...
    public void UpdatePerceptron()
    {
        ...
        waypointPos = getFormationWaypoint(0, 0);

        Vector2 playerPos = new Vector2(player.transform.position.x,player.transform.position.y);
        Vector2 dronePos = new Vector2(drone.transform.position.x, drone.transform.position.y);

        var angle = Clamp0360(drone.transform.eulerAngles.z);
        //Debug.Log(angle);

        var angleRounded = Mathf.Round(angle);
        var currentDiff = (angle - angleRounded);
        var currentQuarterRoundedAngle = Mathf.Round(currentDiff * SC_Angle_Divider) / SC_Angle_Divider; //Mathf.Round(angle * 4) / 4;
        ...
        float pointForwardDirNPCX = (float)(1 * Math.Cos(Math.PI * angle / 180.0)) + dronePos.x; // * Math.PI / 180
        float pointForwardDirNPCY = (float)(1 * Math.Sin(Math.PI * angle / 180.0)) + dronePos.y;

        Vector2 dirForwardNPC = new Vector2(pointForwardDirNPCX - dronePos.x, pointForwardDirNPCY - dronePos.y);

        Vector2 dirRightNPC = new Vector2(pointForwardDirNPCY - dronePos.y,-1* (pointForwardDirNPCX - dronePos.x));

        Vector2 dirNPCToPlayer = new Vector2(playerPos.x - dronePos.x, playerPos.y - dronePos.y);
        ...
        for (int i = 0; i < training.Length; i++)
        {
            ...
            int x = (int)(0.001f * Math.Cos(angleInRadians) + dronePos.x);
            int y = (int)(0.001f * Math.Sin(angleInRadians) + dronePos.y);

            Vector2 dirToGuessedWaypoint = new Vector2(x - dronePos.x, y - dronePos.y);
        ...
        perc.Train(training[count].inputs, training[count].answer0);
