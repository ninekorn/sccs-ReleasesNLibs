...
namespace _sc_core_systems
{
    public class SC_Console_GRAPHICS
    {
                ...
                for (int x = 0; x < _physics_engine_instance_x; x++)
                {
                    for (int y = 0; y < _physics_engine_instance_y; y++)
                    {
                        for (int z = 0; z < _physics_engine_instance_z; z++)
                        {
                            ...
                            offsetPosX = _cube_size_x * 2 * 2; //x between each world instance
                            offsetPosY = _cube_size_y * 2 * 2; //y between each world instance
                            offsetPosZ = _cube_size_z * 2 * 2; //z between each world instance
                            ...
                            {
                                _objects_rigid_static_00[_index][i] = _world_cube_list[i].transform.Component.rigidbody;
                                _objects_static_00[_index][i] = 0;
                                _objects_static_counter_00[_index][i] = 0;

                                _objects_rigid_inactive_00[_index][i] = _world_cube_list[i].transform.Component.rigidbody;
        ...
        void _process_rigidbody_two(RigidBody rigidbody, int x, int y, int z, int count)
        {
            ...
            Matrix.Multiply(ref rotationMatrix, ref translationMatrix, out translationMatrix);
            worldMatrix_instances_cubes[_index][count] = translationMatrix;
            ...
            if (!rigidbody.IsActive)
            {
                if (_has_grabbed_right_swtch == 1)
                {
                    ...
                    Vector3 poser = new Vector3(worldMatrix_instances_cubes[_index][count].M41, worldMatrix_instances_cubes[_index][count].M42, worldMatrix_instances_cubes[_index][count].M43);
