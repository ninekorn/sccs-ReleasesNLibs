...
namespace _sc_core_systems
{
    public class SC_Console_GRAPHICS
    {
                ...
                for (int x = 0; x < _physics_engine_instance_x; x++)
                {
                    for (int y = 0; y < _physics_engine_instance_y; y++)
                    {
                        for (int z = 0; z < _physics_engine_instance_z; z++)
                        {
                            ...
                            _objects_inactive_00[0] = new int[_inst_screen_assets_x * _inst_screen_assets_y * _inst_screen_assets_z];
                            _objects_inactive_counter_00[0] = new int[_inst_screen_assets_x * _inst_screen_assets_y * _inst_screen_assets_z];
                            _objects_rigid_inactive_00[0] = new RigidBody[_inst_screen_assets_x * _inst_screen_assets_y * _inst_screen_assets_z];
                            ...
                            for (int i = 0; i < _some_frame_counter_raycast_00[0].Length; i++)
                            {
                                ...
                                _objects_rigid_inactive_00[0][i] = null;
                                _objects_inactive_00[0][i] = 0;
                                _objects_inactive_counter_00[0][i] = 0;
                ...
                for (int y = 0; y < _physics_engine_instance_y; y++)
                {
                    for (int z = 0; z < _physics_engine_instance_z; z++)
                    {
                        ...
                        for (int i = 0; i < _objects_static_00[_index].Length; i++)
                        {
                            ...
                            {
                                _objects_rigid_static_00[_index][i].IsActive = false;
                                _objects_inactive_00[_index][i] = 0;
                            }
                            else if (_objects_inactive_00[_index][i] == 1)
                            {
                                _objects_rigid_static_00[_index][i].IsActive = true;
                                _objects_inactive_00[_index][i] = 0;
                            }*/
                        }
                    }
                                            ...
                                            if (_world_list[_index].RigidBodies.Count > 0)
                                            {
                                                ...
                                                while (enumerator.MoveNext())
                                                {
                                                    ...
                                                    if (body != null && body.Tag != null)
                                                    {
                                                        ...
                                                        {
                                                            /*if (!body.IsStatic && body.IsActive == false)
                                                            {
                                                                if (handTriggerLeft[0] >= 0.001f)
                                                                {
                                                                    Matrix.Translation(body.Position.X, body.Position.Y, body.Position.Z, out translationMatrix);
                                                                ...
                                                                if (body == _grabbed_body_right)
                                                                {
                                                                    if (_has_grabbed_right_swtch == 2)
                                                                    {
                                                                        ...
                                                                        if (_swtch_hasRotated == 0)
                                                                        {
                                                                            ...
                                                                            worldMatrix_instances_cubes[_index][count] = someRotationFinal;
                                                            ...
                                                            {
                                                                _process_rigidbody_two(body, x, y, z, count);
        ...
        void _process_rigidbody_that_are_currently_activated_or_not(RigidBody rigidbody, int x, int y, int z, int count)
        {
            ...
            /*if (!rigidbody.IsActive)
            {
                bool _boundingBox = _world_list[_index].CollisionSystem.CheckBoundingBoxes(rigidbody, _world_screen_list[_index]._arrayOfInstances[count].transform.Component.rigidbody);
                            ...
                            if (rigidbody.CollisionIsland.Bodies.Count > 0)
                            {
                                if (rigidbody.CollisionIsland.Bodies.Count == 1)
                                {
                                    ...
                                    while (enumerator1.MoveNext())
                                    {
                                        ...
                                        {
                                            int hasbreakeder = 0;
                                            bool _boundingBoxer = _world_list[_index].CollisionSystem.CheckBoundingBoxes(rigidbody, _world_screen_list[_index]._arrayOfInstances[count].transform.Component.rigidbody);
