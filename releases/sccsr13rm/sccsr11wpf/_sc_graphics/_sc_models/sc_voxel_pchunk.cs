...
namespace SCCoreSystems.SC_Graphics
{
    public class sc_voxel_pchunk : ITransform, IComponent
    {
        ...
        int[] map;
        int[] triangles;
        private bool InitializeBuffer(SCCoreSystems.SC_console_directx D3D, int width, int height, int depth, float _restitution, float _kineticfriction, float _staticfriction, float tileSize, int instX, int instY, int instZ, IntPtr windowsHandle, Matrix matroxer, int isTerrain, float offsetPosX, float offsetPosY, float offsetPosZ, SCCoreSystems.SC_console_directx.BodyTag _tag, int minx, int miny, int minz, float diagmaxx, float diagmaxy, float diagmaxz, float diagminx, float diagminy, float diagminz, int ChunkWidth_L, int ChunkWidth_R, int ChunkHeight_L, int ChunkHeight_R, int ChunkDepth_L, int ChunkDepth_R, float planesize, Vector3 offset_pos, float maxDistance, float vertoffsetx, float vertoffsety, float vertoffsetz, Vector3 chunkPosition)
        {
            try
            {
                //_chunk = _chunker;
