...
{
    public class DModelClass4_grid_Tiles                 // 161 lines
    {
        ...
        // Constructor
        public DModelClass4_grid_Tiles() { }
        ...
        // Methods.
        public bool Initialize(SharpDX.Direct3D11.Device device, float _sizeX, float _sizeY, float _sizeZ, int instX,int instY, int instZ) //, string textureFileName
        {

            // Initialize the vertex and index buffer that hold the geometry for the triangle.
            if (!InitializeBuffers(device, _sizeX, _sizeY, _sizeZ, instX, instY, instZ))
                return false;
        ...
        private bool InitializeBuffers(SharpDX.Direct3D11.Device device, float _sizeX, float _sizeY,float _sizeZ, int instX, int instY, int instZ)
        {
            try
            {
                RotationX = 0;
                ...
                DVertexType[] vertices = new[]
                {
                    ...
                    new DVertexType()
                     {
                         position = new Vector3(-1*_sizeX, 0, 1*_sizeZ),
                ...
                DInstanceType[] instances = new DInstanceType[instX * instY * instZ];
                ...
                for (int x = 0; x < instX; x++)
                {
                    for (int y = 0; y < instY; y++)
                    {
                        for (int z = 0; z < instZ; z++)
                        {
                            ...
                            //x + SC_Globals.tinyChunkWidth * (y + SC_Globals.tinyChunkHeight * z)

                            instances[x + instX * (y + instY * z)] = new DInstanceType()
                            {
                                position = new Vector3(x* _sizeX + ((x * _sizeX)), 0 , z * _sizeZ + ((z * _sizeZ)))
                            };
                        }
                    }
                }
