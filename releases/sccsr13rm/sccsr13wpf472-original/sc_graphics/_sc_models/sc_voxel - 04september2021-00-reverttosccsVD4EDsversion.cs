...
namespace SCCoreSystems.SC_Graphics
{
    public class sc_voxel : ITransform, IComponent
    {
        ...
        public bool Initialize(SCCoreSystems.sc_console.SC_console_directx D3D, int width, int height, int depth, float tileSize, int divX, int divY, float _sizeX, float _sizeY, float _sizeZ, Vector4 color, int instX, int instY, int instZ, IntPtr windowsHandle, Matrix matroxer, int isTerrain, float offsetPosX, float offsetPosY, float offsetPosZ, World the_world, float mass, bool is_static, SCCoreSystems.sc_console.SC_console_directx.BodyTag _tag, int minx, int miny, int minz, float diagmaxx, float diagmaxy, float diagmaxz, float diagminx, float diagminy, float diagminz, int ChunkWidth_L_, int ChunkWidth_R_, int ChunkHeight_L_, int ChunkHeight_R_, int ChunkDepth_L_, int ChunkDepth_R_, float voxelgeneralsize, Vector3 offset_pos, float maxDistance, float vertoffsetx, float vertoffsety, float vertoffsetz, int addToWorld, int _voxel_type, int voxelbodyparttype,int somechunkpriminstanceikarmvoxelindex_)
        {

            somechunkpriminstanceikarmvoxelindex = somechunkpriminstanceikarmvoxelindex_;

            planesize = voxelgeneralsize;
            ...
            try
            {
                ...
                try
                {
                    ...
                    _chunk.startBuildingArray(Vector3.Zero,
                                           ...
                                           out triangles,
                                           out map,
                                           voxelgeneralsize, minx, miny, minz, diagmaxx, diagmaxy, diagmaxz, diagminx, diagminy, diagminz, ChunkWidth_L, ChunkWidth_R, ChunkHeight_L, ChunkHeight_L, ChunkDepth_L, ChunkDepth_R, maxDistance, vertoffsetx, vertoffsety, vertoffsetz, voxel_type, voxelbodyparttype, somechunkpriminstanceikarmvoxelindex);
                    ...
                    for (int y = 0; y < instY; y++)
                    {
                        for (int z = 0; z < instZ; z++)
                        {
                            ...
                            if (_addToWorld == 0)
                            {
                                ...
                                //FREE MESH ISSUES WHEN SETTING THAT ONE TOO
                                _cube._LASTPOSITION = _tempMatrix;// Matrix.Identity;
                                //FREE MESH ISSUES WHEN SETTING THAT ONE TOO
                ...
                //Program.MessageBox((IntPtr)0, "" + _arrayOfInstances.Length, "", 0);
