...
namespace SCCoreSystems
{
    public class SC_Console_GRAPHICS
    {
        ...
        float _terrain_size_y = 0.05f; //0.02f too small objects go through
        float _terrain_size_z = 2;


        //main terrain.
        float _floor_size_x = 9;
        float _floor_size_y = 99;
        ...
        public bool Initialize(SCCoreSystems.sc_core.sc_system_configuration configuration, IntPtr windowsHandle, sc_console.sc_console_writer _writer)
        {
            ...
            try
            {
                ...
                _floor = new SC_cube[_physics_engine_instance_x * _physics_engine_instance_y * _physics_engine_instance_z];
                ...
                worldMatrix_instances_floor = new Matrix[_physics_engine_instance_x * _physics_engine_instance_y * _physics_engine_instance_z][];
                ...
                _player_head[tempIndex].Initialize(D3D, D3D.SurfaceWidth, D3D.SurfaceHeight, 0, 1, 1, 1, 0.05f, 0.05f, 0.05f, new Vector4(r, g, b, a), _inst_p_l_shoulder_x, _inst_p_l_shoulder_y, _inst_p_l_shoulder_z, Hwnd, _tempMatroxer, _type_of_cube, offsetPosX, offsetPosY, offsetPosZ, World, _mass, is_static, SC_console_directx.BodyTag.PlayerHead, 9, 9, 9, 18, 13, 13, 40, 39, 40, 39, 40, 39, voxel_general_size, new Vector3(0, 0, 0), 17, vertoffsetx, vertoffsety, vertoffsetz, _addToWorld, voxel_type); //, "terrainGrassDirt.bmp" //0.00035f
                ...
                _object_worldmatrix.M42 = (-_floor_size_y) - 10;
                ...
                _floor[0] = new SC_cube();
                _floor[0].Initialize(D3D, D3D.SurfaceWidth, D3D.SurfaceHeight, 0.05f, 1, 1, _floor_size_x, (_floor_size_y), _floor_size_z, new Vector4(r, g, b, a), _inst_terrain_tile_x, _inst_terrain_tile_y, _inst_terrain_tile_z, Hwnd, _object_worldmatrix, 0, offsetPosX, offsetPosY, offsetPosZ, _world_list[0], SC_console_directx.BodyTag._floor, true, 0, 10, 0, 0, 0); //, "terrainGrassDirt.bmp" //0.00035f

                worldMatrix_instances_floor[0] = new Matrix[_inst_terrain_tile_x * _inst_terrain_tile_y * _inst_terrain_tile_z];
                for (int i = 0; i < worldMatrix_instances_floor[0].Length; i++)
                {
                    worldMatrix_instances_floor[0][i] = _floor[0]._arrayOfInstances[0]._POSITION;
                }
                ...
                _icoSphere.Initialize(D3D.device, 3, 3, 3, new Vector4(0.5f, 0.5f, 0.5f, 1), IcoLightPos , radius);
                ...
                sc_jitter_cloth = new PseudoCloth(World, D3D, windowsHandle, 10, 10, 0.1f, _tempMatroxer);
                ...
                //////////BUOYANCY//////////
                ////////////////////////////
                 _buo = new Jitter.Forces.Buoyancy(World);
                float _size__neg_x = 1.175494351F-38;
                float _size__pos_x = 3.402823466F+38;
                ...
                for (int i = 0; i < _world_list.Length; i++)
                {
                    for (int cu = 0; cu < _world_cube_list[i]._arrayOfInstances.Length;cu++)
                    {
                        _buo.Add(_world_cube_list[i]._arrayOfInstances[cu].transform.Component.rigidbody,3);
                    }

                    for (int cu = 0; cu < _world_voxel_cube_lists[i]._arrayOfInstances.Length; cu++)
                    {
                        _buo.Add(_world_voxel_cube_lists[i]._arrayOfInstances[cu].transform.Component.rigidbody, 3);
                    }

                    
                    _world_list[i].AddBody(_terrain[0]._arrayOfInstances[0].transform.Component.rigidbody);
                    _world_list[i].AddBody(_floor[0]._arrayOfInstances[0].transform.Component.rigidbody);
            ...
            try
            {
                ...
                try
                {
                            ...
                            for (int x = 0; x < _physics_engine_instance_x; x++)
                            {
                                for (int y = 0; y < _physics_engine_instance_y; y++)
                                {
                                    for (int z = 0; z < _physics_engine_instance_z; z++)
                                    {
                                                    ...
                                                    if (body != null && body.Tag != null)
                                                    {
                                                            ...
                                                            (SC_console_directx.BodyTag)body.Tag == SC_console_directx.BodyTag.sc_perko_voxel)
                                                        {
                                                            ...
                                                            if (_buo.FluidBox.Contains(body.BoundingBox) != JBBox.ContainmentType.Disjoint)
                                                            {
                                                                ...
                                                                for (int i = 0; i < positions.Length; i++)
                                                                {
                                                                    ...
                                                                    /*if (_buo.fluidArea == null)
                                                                    {
                                                                        //MessageBox((IntPtr)0, "" + "null0", "Oculus Error", 0);
                                                                        containsCoord = _buo.FluidBox.Contains(ref currentCoord) != JBBox.ContainmentType.Disjoint;
                                                                    }
                                                                    else
                                                                    {
                                                                        has_water_buo_effect = 1;
                                                                        //MessageBox((IntPtr)0, "" + "!null1", "Oculus Error", 0);
                                                                        containsCoord = _buo.fluidArea(ref currentCoord);
                                                                    }*/
                                                                    containsCoord = _buo.FluidBox.Contains(ref currentCoord) != JBBox.ContainmentType.Disjoint;

                                                                    if (containsCoord)
                                                                    {
                                                                        //Console.WriteLine(has_water_buo_effect);
                                                                        has_water_buo_effect = 1;
                                                        ...
                                                        else if ((SC_console_directx.BodyTag)body.Tag == SC_console_directx.BodyTag.sc_perko_voxel)
                                                        {
                                                            ...
                                                            else
                                                            {
                                                                ...
                                                                if (Math.Round(diffXExtra * 10) / 10 < 0.001f && //0.0000001f
                                                                    Math.Round(diffYExtra * 10) / 10 < 0.001f && //0.0000000000001f
                                                                    Math.Round(diffZExtra * 10) / 10 < 0.001f)
                                                                {
                                                                    ...
                                                                    if (currentLinearVelExtra.Length() < 0.000035f && currentAngularVelExtra.Length() < 0.000035f) //0.00035f == 400 approx => 0.00075f == 400 approx
                                                                    {
                                                                ...
                                                                if (handTriggerRight[1] >= 0.001f)
                                                                {
                                                                    ...
                                                                    if (dirToRightHand != null)
                                                                    {
                                                                        ...
                                                                        if (lh_attract_force != JVector.Zero && lh_attract_force != null && lh_attract_force.Length() > 0)
                                                                        {
                                                                            ...
                                                                            if (has_water_buo_effect == 1)
                                                                            {
                                                                                lh_attract_force *= force_4_screen * 100 * handTriggerRight[1] * 0.1f;
                                                                            }
                                                                            else
                                                                            {
                                                                                lh_attract_force *= force_4_screen * 1 * handTriggerRight[1] * 0.1f;
                                                                            }
                                                        ...
                                                        else if ((SC_console_directx.BodyTag)body.Tag == SC_console_directx.BodyTag._floor)
                                                        {
                                                            ...
                                                            Matrix.RotationQuaternion(ref tester, out rigidbody_matrix);
                                                            Matrix.Multiply(ref rigidbody_matrix, ref translationMatrix, out translationMatrix);
                                                            worldMatrix_instances_floor[0][0] = translationMatrix;
                                                            _floor_count++;
                                                        }
                                                        ...
                                                        else if ((SC_console_directx.BodyTag)body.Tag == SC_console_directx.BodyTag.PlayerHead)
                                                        {
                                                            ...
                                                            Quaternion temp_quat;
                                                            Matrix temp_mat = worldMatrix_instances_head[tempIndex][p_head_count];
                                                            Quaternion.RotationMatrix(ref temp_mat, out temp_quat);
                                                            JQuaternion quat = new JQuaternion(temp_quat.X, temp_quat.Y, temp_quat.Z, temp_quat.W);
                                                            JMatrix jmat = JMatrix.CreateFromQuaternion(quat);
                                                            body.Orientation = jmat;
                                                        ...
                                                        else if ((SC_console_directx.BodyTag)body.Tag == SC_console_directx.BodyTag.pseudoCloth)
                                                        {
                                                            ...
                                                            Matrix.RotationQuaternion(ref tester, out rotationMatrix);
                                                            Matrix.Multiply(ref rotationMatrix, ref translationMatrix, out _tempMatrix);
                                                            //_arrayOfClothCubes[clothCounter]._POSITION = _tempMatrix;
                                                            worldMatrix_Cloth_instances[clothCounter] = _tempMatrix;
                                                            clothCounter++;*/
                                                        }
                                                    }
                                        ...
                                        Console.Title = Program._program_name + " => " + "inactive objects: " + _inactive_counter + "              " + " ### " + " Made by Steve Chassé" + " ### ";
                    ...
                    for (int eyeIndex = 0; eyeIndex < 2; eyeIndex++)
                    {
                        ...
                        //TERRAIN SINGLEOBJECT
                        _floor[0].Render(D3D.device.ImmediateContext);
                        _shaderManager.RenderInstancedObject(D3D.device.ImmediateContext, _floor[0].IndexCount, _floor[0].InstanceCount, _floor[0]._POSITION, viewMatrix, _projectionMatrix, _basicTexture.TextureResource, _DLightBuffer, new Vector3(0, 0, 1), _floor[0]);
                        //END OF
                        ...
                        sc_jitter_cloth._cube.Render(D3D.device.ImmediateContext);
                        ...
                        _shaderManager.RenderInstancedObject(D3D.device.ImmediateContext, sc_jitter_cloth._cube.IndexCount, sc_jitter_cloth._cube.InstanceCount, sc_jitter_cloth._cube._POSITION, viewMatrix, _projectionMatrix, _desktopFrame._ShaderResource, _DLightBuffer, new Vector3(0, 0, 1), sc_jitter_cloth._cube); // oculusRiftDir
                        ...
                        /*for (int i = 0; i < _arrayOfClothCubes.Count; i++)
                        {
                            ...
                            //_arrayOfClothCubes[i].RenderInstancedObject(D3D.device.ImmediateContext, _arrayOfClothCubes[i].IndexCount, _arrayOfClothCubes[i].InstanceCount, _arrayOfClothCubes[i]._POSITION, viewMatrix, _projectionMatrix, null, worldMatrix_Terrain_instances, _DLightBuffer, oculusRiftDir);
                            _shaderManager.RenderInstancedObject(D3D.device.ImmediateContext, _arrayOfClothCubes[i].IndexCount, _arrayOfClothCubes[i].InstanceCount, _arrayOfClothCubes[i]._POSITION, viewMatrix, _projectionMatrix, _desktopFrame._ShaderResource, _DLightBuffer, new Vector3(0, 0, 1), _arrayOfClothCubes[i]); // oculusRiftDir
                            ...
                            for (int y = 0; y < _physics_engine_instance_y; y++)
                            {
                                for (int z = 0; z < _physics_engine_instance_z; z++)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        _shaderManager.RenderInstancedObjectsc_perko_voxel(D3D.device.ImmediateContext, _cuber_pelvis1.IndexCount, _cuber_pelvis1.InstanceCount, _cuber_pelvis1._POSITION, viewMatrix, _projectionMatrix, _desktopFrame._ShaderResource, _DLightBuffer_voxel_cube, oculusRiftDir, _cuber_pelvis1);
                                    ...
                                    for (int _iterator = 0; _iterator < _player_rght_hnd[tempIndex]._arrayOfInstances.Length; _iterator++) //
                                    {
                                        ...
                                        _SC_modL_head_BUFFER[0] = new SCCoreSystems.SC_Graphics.sc_voxel.DLightBuffer()
                                        {
                                            ...
                                            lightDirection = dirLight,
                                            padding0 = 7,
                                            lightPosition = new Vector3(_player_head[tempIndex]._POSITION.M41, _player_head[tempIndex]._POSITION.M42, _player_head[tempIndex]._POSITION.M43),
                                            padding1 = 100
                                        };
                                        _rotatingMatrix = finalRotationMatrix;
                                        ...
                                        _rotMatrixer = _player_head[tempIndex]._ORIGINPOSITION;
                                        Quaternion.RotationMatrix(ref _rotMatrixer, out forTest);
                                        ...
                                        MOVINGPOINTER = MOVINGPOINTER + -(direction_feet_up_ori * (_player_head[tempIndex]._total_torso_height * 0.5f));
                                        _rotatingMatrix = finalRotationMatrix;
                                        //Quaternion.RotationMatrix(ref _rotatingMatrix, out otherQuat);
                                        ...
                                        diffNormPosX = (MOVINGPOINTER.X) - _player_head[tempIndex]._ORIGINPOSITION.M41;
                                        diffNormPosY = (MOVINGPOINTER.Y) - _player_head[tempIndex]._ORIGINPOSITION.M42;
                                        diffNormPosZ = (MOVINGPOINTER.Z) - _player_head[tempIndex]._ORIGINPOSITION.M43;


                                        MOVINGPOINTER = MOVINGPOINTER + -(direction_feet_right * (diffNormPosX));
                                        MOVINGPOINTER = MOVINGPOINTER + (direction_feet_up * (_player_torso[tempIndex]._total_torso_height));//MOVINGPOINTER + -(direction_feet_up * (diffNormPosY));
                                        MOVINGPOINTER = MOVINGPOINTER + (direction_feet_up * (_player_head[tempIndex]._total_torso_height * 0.5f));
                                        MOVINGPOINTER = MOVINGPOINTER + -(direction_feet_forward * (diffNormPosZ));
                                        ...
                                        //_player_head[tempIndex]._POSITION = matrixerer;

                                        //_player_head[tempIndex].Render(D3D.device.ImmediateContext);
                                        //_player_head[tempIndex].RenderInstancedObject(D3D.device.ImmediateContext, _player_head[tempIndex].IndexCount, _player_head[tempIndex].InstanceCount, _player_head[tempIndex]._POSITION, viewMatrix, _projectionMatrix, null, worldMatrix_Terrain_instances, _SC_modL_head_BUFFER, oculusRiftDir);
                                        //_player_head[tempIndex]._singleObjectOnly.transform.Component.rigidbody.Position = new JVector(_player_head[tempIndex]._POSITION.M41, _player_head[tempIndex]._POSITION.M42, _player_head[tempIndex]._POSITION.M43);

                                        _player_head[tempIndex].Render(D3D.device.ImmediateContext);
                                        _cuber_pelvis = _player_head[tempIndex];
                                        _cuber_pelvis.Render(D3D.device.ImmediateContext);
                                        _shaderManager._rend_torso(D3D.device.ImmediateContext, _cuber_pelvis.IndexCount, _cuber_pelvis.InstanceCount, _cuber_pelvis._POSITION, viewMatrix, _projectionMatrix, null, _SC_modL_head_BUFFER, oculusRiftDir, _cuber_pelvis);
                                        _player_head[tempIndex]._arrayOfInstances[_iterator].transform.Component.rigidbody.Position = new JVector(_player_head[tempIndex]._POSITION.M41, _player_head[tempIndex]._POSITION.M42, _player_head[tempIndex]._POSITION.M43);

                                        worldMatrix_instances_head[tempIndex][_iterator] = matrixerer;
                                        _player_head[tempIndex]._arrayOfInstances[_iterator].current_pos = matrixerer;
                                    }
                                    ...
                                    _player_head[tempIndex].Render(D3D.device.ImmediateContext);
                                    _shaderManager._rend_torso(D3D.device.ImmediateContext, _player_head[tempIndex].IndexCount, _player_head[tempIndex].InstanceCount, _player_head[tempIndex]._POSITION, viewMatrix, _projectionMatrix, null, _SC_modL_torso_BUFFER, oculusRiftDir, _player_head[tempIndex]);
                ...
                //TERRAIN FLOOR
                _floor[0]._WORLDMATRIXINSTANCES = worldMatrix_instances_floor[0];
                _floor[0]._POSITION = worldMatrix_base[0];

                var cuber_floor = _floor[0];
                ...
                for (int i = 0; i < instancers_floor.Length; i++)
                {
                    ...
                    Quaternion.RotationMatrix(ref sometester_floor[i], out _testQuater);
                ...
                //4 CLOTH CUBE
                sc_jitter_cloth._cube._WORLDMATRIXINSTANCES = worldMatrix_Cloth_instances;
                sc_jitter_cloth._cube._POSITION = worldMatrix_base[0];

                var _sc_jitter_cloth = sc_jitter_cloth;
                var instancers_sc_jitter_cloth = sc_jitter_cloth._cube.instances;
                var sometester_sc_jitter_cloth = sc_jitter_cloth._cube._WORLDMATRIXINSTANCES;

                for (int i = 0; i < instancers_sc_jitter_cloth.Length; i++)
                {
                    float xxx = sometester_sc_jitter_cloth[i].M41;
                    float yyy = sometester_sc_jitter_cloth[i].M42;
                    float zzz = sometester_sc_jitter_cloth[i].M43;

                    sc_jitter_cloth._cube.instances[i].position.X = xxx;
                    sc_jitter_cloth._cube.instances[i].position.Y = yyy;
                    sc_jitter_cloth._cube.instances[i].position.Z = zzz;
                    sc_jitter_cloth._cube.instances[i].position.W = 1;
                    Quaternion.RotationMatrix(ref sometester_sc_jitter_cloth[i], out _testQuater);

                    var dirInstance = _newgetdirforward(_testQuater);
                    sc_jitter_cloth._cube.instancesDataForward[i].rotation.X = dirInstance.X;
                    sc_jitter_cloth._cube.instancesDataForward[i].rotation.Y = dirInstance.Y;
                    sc_jitter_cloth._cube.instancesDataForward[i].rotation.Z = dirInstance.Z;
                    sc_jitter_cloth._cube.instancesDataForward[i].rotation.W = 1;

                    dirInstance = -_newgetdirleft(_testQuater);
                    sc_jitter_cloth._cube.instancesDataRIGHT[i].rotation.X = dirInstance.X;
                    sc_jitter_cloth._cube.instancesDataRIGHT[i].rotation.Y = dirInstance.Y;
                    sc_jitter_cloth._cube.instancesDataRIGHT[i].rotation.Z = dirInstance.Z;
                    sc_jitter_cloth._cube.instancesDataRIGHT[i].rotation.W = 1;

                    dirInstance = dirInstance = _newgetdirup(_testQuater);
                    sc_jitter_cloth._cube.instancesDataUP[i].rotation.X = dirInstance.X;
                    sc_jitter_cloth._cube.instancesDataUP[i].rotation.Y = dirInstance.Y;
                    sc_jitter_cloth._cube.instancesDataUP[i].rotation.Z = dirInstance.Z;
                    sc_jitter_cloth._cube.instancesDataUP[i].rotation.W = 1;
                }
                //END OF
                    ...
                    for (int y = 0; y < _physics_engine_instance_y; y++)
                    {
                        for (int z = 0; z < _physics_engine_instance_z; z++)
                        {
                            ...
                            */
                            ...
                            //PHYSICS HEAD
                            _player_head[indexer00]._WORLDMATRIXINSTANCES = worldMatrix_instances_head[indexer00];
                            _player_head[indexer00]._POSITION = worldMatrix_base[0];

                            var cuber_head = _player_head[indexer00];
                            var instancers_head = cuber_head.instances;
                            var sometester_head = cuber_head._WORLDMATRIXINSTANCES;

                            for (int i = 0; i < instancers_head.Length; i++)
                            {
                                float xxx = sometester_head[i].M41;
                                float yyy = sometester_head[i].M42;
                                float zzz = sometester_head[i].M43;

                                cuber_head.instances[i].position.X = xxx;
                                cuber_head.instances[i].position.Y = yyy;
                                cuber_head.instances[i].position.Z = zzz;
                                cuber_head.instances[i].position.W = 1;
                                Quaternion _testQuater;
                                Quaternion.RotationMatrix(ref sometester_head[i], out _testQuater);

                                var dirInstance = _newgetdirforward(_testQuater);
                                cuber_head.instancesDataForward[i].rotation.X = dirInstance.X;
                                cuber_head.instancesDataForward[i].rotation.Y = dirInstance.Y;
                                cuber_head.instancesDataForward[i].rotation.Z = dirInstance.Z;
                                cuber_head.instancesDataForward[i].rotation.W = 1;

                                dirInstance = -_newgetdirleft(_testQuater);
                                cuber_head.instancesDataRIGHT[i].rotation.X = dirInstance.X;
                                cuber_head.instancesDataRIGHT[i].rotation.Y = dirInstance.Y;
                                cuber_head.instancesDataRIGHT[i].rotation.Z = dirInstance.Z;
                                cuber_head.instancesDataRIGHT[i].rotation.W = 1;

                                dirInstance = dirInstance = _newgetdirup(_testQuater);
                                cuber_head.instancesDataUP[i].rotation.X = dirInstance.X;
                                cuber_head.instancesDataUP[i].rotation.Y = dirInstance.Y;
                                cuber_head.instancesDataUP[i].rotation.Z = dirInstance.Z;
                                cuber_head.instancesDataUP[i].rotation.W = 1;
                            }
