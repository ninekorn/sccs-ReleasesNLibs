                ...
                for (int x = 0; x < _physics_engine_instance_x; x++)
                {
                    for (int y = 0; y < _physics_engine_instance_y; y++)
                    {
                        for (int z = 0; z < _physics_engine_instance_z; z++)
                        {
                            ...
                            offsetPosX = _cube_size_x * 2; //x between each world instance
                            offsetPosY = _cube_size_y * 2; //y between each world instance
                            offsetPosZ = _cube_size_z * 2; //z between each world instance
        ...
        private sc_message_object.sc_message_object[] Render(sc_message_object.sc_message_object[] _main_received_object)
        {
            ...
            try
            {
                ...
                try
                {
                            ...
                            for (int x = 0; x < _physics_engine_instance_x; x++)
                            {
                                for (int y = 0; y < _physics_engine_instance_y; y++)
                                {
                                    for (int z = 0; z < _physics_engine_instance_z; z++)
                                    {
                                                ...
                                                while (enumerator.MoveNext())
                                                {
                                                    ...
                                                    if (body != null && body.Tag != null)
                                                    {
                                                        if ((SC_console_directx.BodyTag)body.Tag == SC_console_directx.BodyTag.physicsInstancedScreen)
                                                        {
                                                            ...
                                                            Matrix.Multiply(ref rigidbody_matrix, ref translationMatrix, out translationMatrix);
                                                            worldMatrix_instances_screens[0][0] = translationMatrix;
                                                            _world_screen_list[0]._arrayOfInstances[0].current_pos = worldMatrix_instances_screens[0][0];
                                        ...
                                        Console.Title = "inactive objects: " + _inactive_counter + " ### " + "static objects: " + _static_counter;
                    ...
                    var lightDirection = _getDirection(Vector3.ForwardRH, otherQuat);
                    ...
                    Vector3 lightpos = new Vector3(0, 10, 0);// new Vector3(_player_rght_hnd[0]._arrayOfInstances[0].current_pos.M41, _player_rght_hnd[0]._arrayOfInstances[0].current_pos.M42, _player_rght_hnd[0]._arrayOfInstances[0].current_pos.M43);
