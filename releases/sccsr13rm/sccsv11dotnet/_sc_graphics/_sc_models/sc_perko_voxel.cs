...
namespace _sc_core_systems.SC_Graphics
{
    ...
    public class sc_perko_voxel
    {
        ...
        //if i can code this part, i am also good to go for the insanely more powerful jitter 3d index. i am not sure i can code this.
        //at least 4 are repetitive but i don't mind having them there because it was easier to calculate that way.
        //NOT WORKING YET. JUNK CODE. godamnit i need the 2d sebastien lague node distance translated to 3d.
        private float _check_node_distance(Vector3 nodeA, Vector3 nodeB)
        {
            var dstX = Math.Abs((nodeA.X) - (nodeB.X));
            var dstY = Math.Abs((nodeA.Y) - (nodeB.Y));
            var dstZ = Math.Abs((nodeA.Z) - (nodeB.Z));
            ...
            else
            {
                ...
                var part_01 = 14 * dstY;
                var part_02 = 14 * dstZ;
                return part_00 + part_01 + part_02;
            }
        }
        ...
        public void startBuildingArray(Vector3 currentPosition, out SC_cube.DVertex[] vertexArray, out int[] triangleArray, out int[] mapper, float _planeSize, int minx, int maxx, int miny, int maxy, int minz, int maxz, int _ChunkWidth_L, int _ChunkWidth_R, int _ChunkHeight_L, int _ChunkHeight_R, int _ChunkDepth_L, int _ChunkDepth_R, float maxDistance, float vert_offset_x, float vert_offset_y, float vert_offset_z, int voxel_type) //, out int vertexNum, out int indicesNum //Vector3 currentPosition, out Vector3[] vertexArray, out int[] indicesArray, 
        {
