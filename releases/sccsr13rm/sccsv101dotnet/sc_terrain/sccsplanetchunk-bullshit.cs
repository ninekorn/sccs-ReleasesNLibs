...
namespace SCCoreSystems.SC_Graphics
{
    ...
    public class sccsplanetchunk
    {
        ...
        private float radiusplanetcoreend = 1.0f;
        private float radiusplanetcavesstart = 2.0f;
        private float radiusplanetcavesend = 3.0f;
        private float radiusplanetcruststart = 4.0f;
        private float radiusplanetcrustend = 5.0f;
        private float radiusplanetmountainstart = 6.0f;
        private float radiusplanetmountainend = 7.0f;
        ...
        public struct sc_chunkplanet_node
        {
        ...
        private void CreateSpikeGlycoProteinCOVID19(Vector3 center, int x, int y, int z) //, float min, float max //ref BlockData[,] blocks, TerrainType terrain
        {
            ...
            sc_chunkplanet_node _sc_node;
            List<sc_chunkplanet_node> _sc_node_list;
            ...
            for (float i = 0; i < _spike_max_length * 1.0f;) //_spike_max_length //(float)Math.Round(_spike_max_length * 0.75f)
            {
                ...
                _sc_node_list = new List<sc_chunkplanet_node>();
                ...
                for (int xx = -1; xx <= 1; xx++)
                {
                    for (int yy = -1; yy <= 1; yy++)
                    {
                        for (int zz = -1; zz <= 1; zz++)
                        {
                            ...
                            _sc_node = new sc_chunkplanet_node();
        ...
        public void startBuildingArray(Vector3 currentPosition, float _planeSize, int minx, int miny, int minz, float diagmaxx, float diagmaxy, float diagmaxz, float diagminx, float diagminy, float diagminz, int _ChunkWidth_L, int _ChunkWidth_R, int _ChunkHeight_L, int _ChunkHeight_R, int _ChunkDepth_L, int _ChunkDepth_R, float maxDistance, float vert_offset_x, float vert_offset_y, float vert_offset_z, int voxel_type, sccsproceduralplanetbuilder _componentPar,Vector3 parentPosition,Vector3 realChunkPos) //, out int vertexNum, out int indicesNum //Vector3 currentPosition, out Vector3[] vertexArray, out int[] indicesArray, 
        {
            ...
            for (int x = -ChunkWidth_L; x <= ChunkWidth_R; x++)
            {
                for (int y = -ChunkHeight_L; y <= ChunkHeight_R; y++)
                {
                    for (int z = -ChunkDepth_L; z <= ChunkDepth_R; z++)
                    {
                        ...
                        /*float distance = Vector3.Distance(currentPosition, parentPosition);

                        if (_index >= 0 && _index < _max)
                        {
                            if (distance <= (radiusplanetcoreend + realplanetwidth))
                            {
                                map[_index] = 1;
                            }
                        }*/
                        ...
                        /*float distance = 0;
                        distance = Vector3.Distance(currentPosition, parentPosition);

                        if (distance < ((realplanetwidth) * 0.25f)) // 0.35f
                        {
                            if (_index >= 0 && _index < _max)
                            {
                                map[_index] = 1;
                        ...
                        Vector3 pos = new Vector3(posX, posY, posZ) + currentPosition;

                        float distance = Vector3.Distance(pos, parentPosition);

                        if (distance <= radiusplanetcoreend* (ChunkWidth_L) * 0.75f)
                        {
                            if (_index >= 0 && _index < _max)
                            {
                                map[_index] = 1;
            ...
            for (int x = -ChunkWidth_L; x <= ChunkWidth_R; x++)
            {
                for (int y = -ChunkHeight_L; y <= ChunkHeight_R; y++)
                {
                    for (int z = -ChunkDepth_L; z <= ChunkDepth_R; z++)
                    {
                        ...
                        if (block == 0) continue;
                        {
                            DrawBrick(xx, yy, zz, currentPosition, x, y, z); // xx, yy, zz
                        }
                    }
                }
            }
        ...
        public void DrawBrick(int x, int y, int z, Vector3 position, int xxx, int yyy, int zzz) //public void DrawBrick(int x, int y, int z, int xx, int yy, int zz)
        {
            //Vector3 start = new Vector3(xxx * planeSize, yyy * planeSize, zzz * planeSize);
            ...
            //RIGHTFACE
            if (x != (ChunkWidth_L + ChunkWidth_R + 1) - 1)
            {
            ...
            else if (x == (ChunkWidth_L + ChunkWidth_R + 1) - 1)
            {
            ...
            else if (x == 0)
            {
                if (componentParent.getChunk((int)(_zerochunkpos.X - realplanetwidth), (int)(_zerochunkpos.Y), (int)(_zerochunkpos.Z)) != null)
                {
                    ...
                    if (chunkdata != null)
                    {
                        ...
                        if (comp != null)
                        {
                            if (comp._chunk.IsTransparent((ChunkWidth_L + ChunkWidth_R + 1) - 1, y, z))
                            {
                                offset1 = Vector3.BackwardLH * planeSize;
                                offset2 = Vector3.Down * planeSize;
                                createleftFace(start + Vector3.Up * planeSize + Vector3.ForwardLH * planeSize, offset1, offset2);
            ...
            if (z == 0 && componentParent.getChunk((int)(_zerochunkpos.X), (int)(_zerochunkpos.Y), (int)(_zerochunkpos.Z - realplanetwidth)) != null)
            {
                ...
                if (chunkdata != null)
                {
                    ...
                    if (comp != null)
                    {
                        if (comp._chunk.IsTransparent(x, y, (ChunkDepth_L + ChunkDepth_R + 1) - 1))
                        {
            ...
            //BACKFACE
            if (z == (ChunkWidth_L + ChunkWidth_R + 1) - 1 && componentParent.getChunk((int)(_zerochunkpos.X), (int)(_zerochunkpos.Y), (int)(_zerochunkpos.Z + realplanetwidth)) != null)
            {
                mainChunk chunkdata = componentParent.getChunk((int)(_zerochunkpos.X), (int)(_zerochunkpos.Y), (int)(_zerochunkpos.Z + realplanetwidth));
            ...
            else if (z != (ChunkWidth_L + ChunkWidth_R + 1) - 1)
            {
                //BACKFACE
                if (IsTransparent(x, y, z + 1))
                {
            ...
            //TOPFACE
            if (y == (ChunkHeight_L + ChunkHeight_R + 1) - 1 && componentParent.getChunk((int)(_zerochunkpos.X), (int)(_zerochunkpos.Y + realplanetwidth), (int)(_zerochunkpos.Z)) != null)
            {
            ...
            else if (y != (ChunkHeight_L + ChunkHeight_R + 1) - 1)
            {
                //TOPFACE
                if (IsTransparent(x, y + 1, z))
                {
            ...
            if (y == 0 && componentParent.getChunk((int)(_zerochunkpos.X), (int)(_zerochunkpos.Y - realplanetwidth), (int)(_zerochunkpos.Z)) != null)
            {
                ...
                if (chunkdata != null)
                {
                    ...
                    if (comp != null)
                    {
                        if (comp._chunk.IsTransparent(x, (ChunkHeight_L + ChunkHeight_R + 1) - 1, z))
                        {
                            offset1 = Vector3.Right * planeSize;
                            offset2 = Vector3.ForwardLH * planeSize;
                            createBottomFace(start, offset1, offset2);
