                        ...
                        for (int z = 0; z < instZ; z++)
                        {
                            ...
                            else if (_addToWorld == 2)
                            {
                                ...
                                if (Program.usejitterphysics == 1)
                                {
                                    ...
                                    if (_tag == SC_console_directx.BodyTag._voxel_spheroid)
                                    {
                                        _cube.transform.Component.rigidbody = new RigidBody(new SphereShape((_sizeY * ((ChunkWidth_L + ChunkWidth_R) * 0.35f * planesize)) * 87.25f), _the_world.RigidBodies.Count + 1, 0);
                                    }
                                    else
                                    {
                                        _cube.transform.Component.rigidbody = new RigidBody(new BoxShape(((ChunkWidth_L + ChunkWidth_R)) * _sizeX * planesize, ((ChunkHeight_L + ChunkHeight_R)) * _sizeY * planesize, ((ChunkDepth_L + ChunkDepth_R)) * _sizeZ * planesize), _the_world.RigidBodies.Count + 1, 0);
                                    }
                                    //_cube.transform.Component.rigidbody = new RigidBody(new SphereShape(1)); //_sizeY * (ChunkWidth * 0.35f //39.2625f
                                    //_cube.transform.Component.rigidbody = new RigidBody(new SphereShape((_sizeY * ((ChunkWidth_L+ ChunkWidth_R) * 0.35f * planesize)) * 87.25f)); //_sizeY * (ChunkWidth * 0.35f //39.2625f
