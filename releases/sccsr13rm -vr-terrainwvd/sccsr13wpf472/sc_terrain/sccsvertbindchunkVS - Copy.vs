...
{ 
	...
	{	
		...
		*/
		...
		/*
		//float normx = dot(input.normal, float3(1, 0.45, 0.65));
		float dirr = dot(float3(ovrdirr.x,ovrdirr.y,ovrdirr.z), input.normal.xyz);
		float diru = dot(float3(ovrdiru.x,ovrdiru.y,ovrdiru.z), input.normal.xyz);
		float dirf = dot(float3(ovrdirf.x,ovrdirf.y,ovrdirf.z), input.normal.xyz);

		if(dirr > 0 && diru > 0 && dirf > 0)
		{
			MOVINGPOINT = MOVINGPOINT + (-rightDir * diffX);
			MOVINGPOINT = MOVINGPOINT + (upDir * diffY);
			MOVINGPOINT = MOVINGPOINT + (forwardDir * diffZ);
		...
		*/
		...
		/*
		if(Dot(float3(-rightDir.x,-rightDir.y,-rightDir.z),float3(ovrdirr.x,ovrdirr.y,ovrdirr.z)) > 0)
		{
			MOVINGPOINT = MOVINGPOINT + (-rightDir * diffX);
			MOVINGPOINT = MOVINGPOINT + (upDir * diffY);
			MOVINGPOINT = MOVINGPOINT + (forwardDir * diffZ);
		...
		/*else if(Dot(float3(-rightDir.x,-rightDir.y,-rightDir.z),float3(ovrdirr.x,ovrdirr.y,ovrdirr.z)) < 0 && Dot(float3(-pDir.x,upDir.y,upDir.z),float3(ovrdiru.x,ovrdiru.y,ovrdiru.z)) < 0 && Dot(float3(forwardDir.x,forwardDir.y,forwardDir.z),float3(-rightDir.x,-rightDir.y,-rightDir.z)) < 0)
